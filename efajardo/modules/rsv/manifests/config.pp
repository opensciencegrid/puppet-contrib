# == Class: rsv::config
#
# This module manages the basic configuration for the rsv
#
# === Parameters
# [*gridftp_hosts*]
...
{
  case "${htcondor_ce_hosts}" {
   ''   : { $htcondor_gateway_enabled = False } # The htcondor_ce_hosts is empty
   /^./ : { $htcondor_gateway_enabled = True }  # The htcondor_ce_hosts is not empty
  }
  case "${gram_ce_hosts}" {
    ''   : { $gram_gateway_enabled = False } # The gram_ce_hosts is empty
    /^./ : { $gram_gateway_enabled = True }  # The gram_ce_hosts is not empty
  }
  file { '10-gateway.ini':
    ensure  => present,
    path    => '/etc/osg/config.d/10-gateway.ini',
    content => template('rsv/10-gateway.ini.erb'),
  } 
  # Determine the $ce_hosts in the config file by joinning the gram_ce_hosts and htcondor_ce_hosts
  # The case is if in case one of them or both are empty not to let
  case "${htcondor_ce_hosts}, ${gram_ce_hosts}"{
   /^,./   : { $ce_hosts = "${gram_ce_hosts}"} # if the htcondor_ce_hosts was empty
   /., *$/  : { $ce_hosts = "${htcondor_ce_hosts}"} # if the gram_ce_hosts was empty
   /^,$/   : { $ce_hosts = "" } # In case both are empty
   default : { $ce_hosts = "${htcondor_ce_hosts},${gram_ce_hosts}"}
  }

  file { '30-rsv.ini':
    ensure  => present,
    path    => '/etc/osg/config.d/30-rsv.ini',
    content => template('rsv/30-rsv.ini.erb'),
  }

  exec { 'osg-configure -v':
    command => 'osg-configure -v',
    path    => ["/usr/bin", "/usr/sbin"]
  }

  exec { 'osg-configure -c':
    command => 'osg-configure -c',
    path    => ["/usr/bin", "/usr/sbin"]
  }
  
  File['30-rsv.ini'] -> Exec['osg-configure -v']
  Exec['osg-configure -v'] -> Exec['osg-configure -c']
}
